...
Friendship Structure attention mechanism.
'''
...
def parse_args():
    ...
    parser.add_argument('--l2', nargs='?', default=1e-10,
                        help="Regularization for user and item embeddings.")
    parser.add_argument('--l1l2', nargs='?', default=0.1,
                        help="Regularization for user and item embeddings.")
    ...
    parser.add_argument('--num_users', type=int, default=52167,
                        help='Num of users.')
    parser.add_argument('--tmp_dim', type=int, default=150,
                        help='Num of users.')
...
class get_model_NeuMF(object):
    ...
    def build_var(self):
                with tf.variable_scope('MLP'):
                    ...
                    self.mlp_attention_weight_2 = tf.get_variable("MLP_Attention_weight_2",
                                                                  initializer=self.initializer([self.tmp_dim, self.n_friends]))
                    self.mlp_attention_bias_2 = tf.get_variable("MLP_Attention_bias_2",
                                                                initializer=self.initializer([self.n_friends]))
                    #MLP
                ...
                with tf.variable_scope('GMF'):
                    #Item embedding
                    self.MF_Embedding_Item= tf.get_variable('MF_Embedding_Item',initializer=self.initializer([self.n_classes,self.gmf_latent_dim]))
                    ...
                    self.MF_attention_weight_2= tf.get_variable("MF_Attention_weight_2", initializer=self.initializer([self.tmp_dim, self.n_friends]))
                    self.MF_attention_bias_2 = tf.get_variable("MF_Attention_bias_2", initializer=self.initializer([self.n_friends]))



                with tf.variable_scope('merge'):
                    self.Prediction_weight = tf.get_variable('Prediction_weight',
                                                                initializer=self.initializer([self.gmf_latent_dim+self.mlp_layers[-1], 1]))
    ...
    def build_model(self):
            with tf.device('/gpu:0'):
                    with tf.variable_scope('MLP'):
                        ...
                        MLP_User_Attention_Vector_0=self.activation(tf.add(tf.matmul(self.user_features_input, self.mlp_attention_weight_1), self.mlp_attention_bias_1))  #None*64
                        MLP_User_Attention_Vector = tf.sigmoid(tf.add(tf.matmul(MLP_User_Attention_Vector_0, self.mlp_attention_weight_2),self.mlp_attention_bias_2))  # None*friends
                        MLP_User_Attention_Vector_1=tf.nn.relu(tf.subtract(self.user_features_input,MLP_User_Attention_Vector))  ##>0 means original, otherwise origin=0
                        MLP_User_Attention_Vector_1=self.user_features_input-MLP_User_Attention_Vector_1  ###retrivial.
                        MLP_User_Latent_Input=tf.multiply(self.user_features_input, MLP_User_Attention_Vector_1)
                        MLP_User_Latent_Vector = self.activation(tf.add(tf.matmul( MLP_User_Latent_Input, self.mlp_layer_weight[0]), self.mlp_layer_bias[0]))

                        mlp_predict_vector = tf.concat([MLP_Item_Latent_Vector,MLP_User_Latent_Vector],1)
                        #MLP
                        num_layer = len(self.mlp_layers)
                        for idx in xrange(1,num_layer):
                    ...
                    with tf.variable_scope('GMF'):
                        ...
                        MF_User_Attention_Vector_0 = self.activation(tf.add(tf.matmul(self.user_features_input, self.MF_attention_weight_1),self.MF_attention_bias_1))  # None*64
                        MF_User_Attention_Vector= tf.sigmoid(tf.add(tf.matmul(MF_User_Attention_Vector_0, self.MF_attention_weight_2),self.MF_attention_bias_2))  # None*64
                        MF_User_Latent_Input = tf.multiply(self.user_features_input, MF_User_Attention_Vector)
                        MF_User_Latent_Vector = self.activation(tf.add(tf.matmul(MF_User_Latent_Input, self.MF_layer_weight), self.MF_layer_bias))
