...
def parse_args():
    ...
    parser.add_argument('--l1', nargs='?', default=1e-10,
                        help="Regularization for user and item embeddings.")
    parser.add_argument('--l2', nargs='?', default=1e-10,
                        help="Regularization for user and item embeddings.")
    parser.add_argument('--l1l2', nargs='?', default=0.01,
                        help="Regularization for user and item embeddings.")
    ...
    parser.add_argument('--num_users', type=int, default=11987,
                        help='Num of users.')
    ...
    parser.add_argument('--log_on', type=str,
                        default='[\'dataset\',\'pre_load\', \'learning_rate\',\'num_factors\',\'mlp_layers\',\'pre_target\',\'l1\',\'l2\',\'l1l2\']',
...
class get_model_NeuMF(object):
    ...
    def __init__(self, args, sess):
        ...
        #grads = [tf.clip_by_norm(grad, self.max_grad_norm) for grad in grads[:len(var_list)]]
    ...
    def build_var(self):
                with tf.variable_scope('Embedding'):
                    ##########################MLP
                    # Item embedding
                    self.MLP_Embedding_Item = tf.get_variable('MLP_Embedding_Item', initializer=self.initializer(
                        [self.n_classes, self.mlp_layers[0] / 2]))
                    #User
                    self.MLP_Embedding_User_Weight = tf.get_variable("MLP_USER_weight" , initializer=self.initializer([self.n_friends, self.mlp_layers[0]/ 2]))
                    self.MLP_Embedding_User_Bias = tf.get_variable("MLP_USER_BIAS", initializer=self.initializer(
                        [self.mlp_layers[0]/ 2]))

                    ##########################GMF
                    # Item embedding
                    self.MF_Embedding_Item = tf.get_variable('MF_Embedding_Item', initializer=self.initializer(
                        [self.n_classes, self.gmf_latent_dim]))
                    #User
                    self.MF_Embedding_User_Weight = tf.get_variable('MF_user_weight', initializer=self.initializer(
                        [self.n_friends, self.gmf_latent_dim]))
                    self.MF_Embedding_User_Bias = tf.get_variable('MF_user_bias',
                                                         initializer=self.initializer([self.gmf_latent_dim]))
    ...
    def build_model(self):
            with tf.device('/gpu:0'):
                    with tf.variable_scope('Embedding'):
                        ...
                        MLP_User_Latent_Vector = self.activation(tf.add(tf.matmul(self.user_features_input, self.MLP_Embedding_User_Weight), self.MLP_Embedding_User_Bias))
                        ...
                        MF_User_Latent_Vector = self.activation(
                            tf.add(tf.matmul(self.user_features_input, self.MF_Embedding_User_Weight), self.MF_Embedding_User_Bias))
                        mf_predict_vector = tf.multiply(MF_Item_Latent_Vector, MF_User_Latent_Vector)

                    with tf.variable_scope('MLP'):
                        #MLP
                        num_layer = len(self.mlp_layers)
                        for idx in xrange(0,num_layer-1):
    ...
    def save(self,out_file):
        saver=tf.train.Saver([self.MLP_Embedding_User_Weight,self.MLP_Embedding_User_Bias,self.MF_Embedding_User_Weight,
                              self.MF_Embedding_User_Bias])
        saver.save(self.sess, out_file)
        print("saved!")
