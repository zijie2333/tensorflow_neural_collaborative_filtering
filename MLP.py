...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='Legalization of Abortion_933',
                        help='Choose a dataset.')
    parser.add_argument('--epochs', type=int, default=100,
                        help='Number of epochs.')
    parser.add_argument('--batch_size', type=int, default=32,
                        help='Batch size.')
    parser.add_argument('--mlp_layers', nargs='?', default='[64,64,32,16,8]',
                        help="Size of each layer. Note that the first layer is the concatenation of user and item embeddings. So layers[0]/2 is the embedding size.")
    parser.add_argument('--learning_rate', type=float, default=0.01,
                        help='Learning rate.')
    parser.add_argument('--l1', nargs='?', default=1e-5,
                        help="Regularization for user and item embeddings.")
    parser.add_argument('--l2', nargs='?', default=1e-5,
                        help="Regularization for user and item embeddings.")
    parser.add_argument('--l1l2', nargs='?', default=1,
                        help="Regularization for user and item embeddings.")
    ...
    parser.add_argument('--num_users', type=int, default=23735,
                        help='Num of users.')
    parser.add_argument('--pre_target', type=bool, default=True,
    ...
    parser.add_argument('--log_on', type=str,
                        default='[\'dataset\', \'learning_rate\',\'mlp_layers\',\'pre_target\',\'l1\',\'l2\',\'l1l2\']',
                        help='the parameter of fine-tunning')
    return parser.parse_args()


class get_model_MLP(object):
    """Friends_Model
    """
    ...
    def build_var(self):
                ...
                with tf.variable_scope('MLP'):
                    ...
                    #Predict
                    self.MLP_Prediction_weight = tf.get_variable('MLP_Prediction_weight',initializer=self.initializer([self.mlp_layers[-1], 1]))
                    self.MLP_Prediction_bias = tf.get_variable('MLP_Prediction_bias', initializer=self.initializer([1]))
    ...
    def build_model(self):
            with tf.device('/gpu:0'):
                    ...
                    with tf.variable_scope('MLP'):
                        ...
                        predict_vector = tf.concat([MLP_Item_Latent_Vector,MLP_User_Latent_Vector],1)
                        #MLP
                        num_layer = len(self.mlp_layers)
                        for idx in xrange(1,num_layer):
                            predict_vector=tf.add(tf.matmul(predict_vector, self.mlp_layer_weight[idx]), self.mlp_layer_bias[idx])
                            predict_vector = self.activation(predict_vector)

                        #Prediction
                        prediction = tf.nn.sigmoid(tf.add(tf.matmul(predict_vector, self.MLP_Prediction_weight), self.MLP_Prediction_bias))
                    return prediction
...
if __name__ == '__main__':
    ...
    save_path='Pretrain/%s/MLP/'%args.dataset
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    model_out_file = save_path+'%s_MLP_%d.h5' % (args.dataset, time())
    ...
    logging.basicConfig(level=logging.INFO,
                        ...
                        filename=os.path.join(args.log_path, "MLP_%s.txt" % log_name))
    print(args)
    logging.info("MLP_%s!============================================" % args.dataset)
    logging.info(args)
...
plt.savefig(fig_path+'%.1f_MLP1_%s.png' % (time(), log_name))
...
# plt.show()
plt.savefig(fig_path+'%.1f_MLP2_%s.png' % (time(), log_name))
